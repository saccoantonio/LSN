...
void initiating_rnd_wline(Random& rnd, int rank){
    int seed[4];
    int p1, p2;

    // Rank 0 legge i file una volta sola
    ifstream Primes("Primes");
    if (Primes.is_open()){
    ...
    if (input.is_open()){
        while (!input.eof()){
            ...
            if (property == "RANDOMSEED"){
                input >> seed[0] >> seed[1] >> seed[2] >> seed[3];
                // variare i seed con il rank
                for (int i = 0; i < 4; ++i) seed[i] += rank * 23;
                rnd.SetRandom(seed, p1, p2);
            }
        }
        input.close();
    } else cerr << "PROBLEM: Unable to open seed.in" << endl;
} //Questo evita accessi simultanei e ti assicura che ogni processo abbia un seed diverso
