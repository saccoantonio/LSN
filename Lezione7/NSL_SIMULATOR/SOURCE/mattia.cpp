...
void System :: move(int i){ // Propose a MC move for particle i
  if(_sim_type == 3){ //Gibbs sampler for Ising, modifing for ex6
    double delta_E;
    delta_E = 2.0*( _J * (_particle(this->pbc(i-1)).getspin() + _particle(this->pbc(i+1)).getspin() ) + _H );     // TO BE FIXED IN EXERCISE 6
  ...
  if(_restart){
    ...
    if(cinf.is_open()){
      ...
      for(int i=0; i<_npart; i++){
        ...
        _particle(i).setpositold(0, this->pbc(_side(0)*xold, 0)); //METTERE SETPOSITION SENZA OLD NEL 04.3??
    ...
    vec sumv(_ndim);
    sumv.zeros();
    for (int i=0; i<_npart; i++){       //MAXWELL BOLTZAMN ES4
    ...
    /*double _initial_velocity = sqrt(3.0*_temp); //added on ex4 for delta distribution 
    for (int i=0; i<_npart; i+=6){
      vx(i) = _initial_velocity;
      vx(i+1) = - _initial_velocity;
      vy(i+2) = _initial_velocity;
      vy(i+3) = - _initial_velocity;
      vz(i+4) = _initial_velocity;
      vz(i+5) = - _initial_velocity;
    }
    for(int i=0; i<_npart; i++){
      sumv(0) += vx(i);
      sumv(1) += vy(i);
    ...
    //finish added part, comment eventually
  ...
  if (input.is_open()){
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){
        ...
        index_property++;
        _vtail = 8*M_PI*_rho/(9*pow(_r_cut,9)) - 8*M_PI*_rho/(3*pow(_r_cut,3)); ///mod in ex7
        ...
        index_property++;
        _ptail = 32*M_PI*_rho*(1/(9*pow(_r_cut,9))-1/(6*pow(_r_cut,3))); //mod in ex7
...
void System :: measure(){ // Measure properties
  ...
  int bin;
  int bin_g, norm_g; // added for 4x7
  vec distance;
  distance.resize(_ndim);
  ...
  double virial=0.0;
  std::vector<int> bin_counts(_n_bins_v, 0); //added for ex4 
  if (_measure_penergy or _measure_pressure or _measure_gofr) {
    for (int i=0; i<_npart-1; i++){
      for (int j=i+1; j<_npart; j++){
        ...
        dr = sqrt( dot(distance,distance) );
        // ex7 modifications
        bin_g = int(dr/_bin_size);
        norm_g = _rho*_npart*(4./3.)*M_PI*(pow(bin_g*_bin_size + _bin_size,3) - pow(bin_g*_bin_size,3));
        if(bin_g < _n_bins){
        _measurement(_index_gofr + bin_g) += 2./norm_g;
        }
        if(dr < _r_cut){
  ...
  if(_measure_pofv){
    ...
    for(int i=0; i<_npart; i++){
      ...
      bin = int(speed/_bin_size_v);
      if(bin>0 && bin < _n_bins_v){
        bin_counts[bin]++;
        _included ++;
      }
    }
    double norm = 1.0/(double(_included)*_bin_size_v);
    for(int i=0; i<_n_bins_v; i++){
      _measurement(_index_pofv + i) = bin_counts[i]*norm;
    }
  }
  // POTENTIAL ENERGY //////////////////////////////////////////////////////////
  if (_measure_penergy){
  ...
  // SPECIFIC HEAT /////////////////////////////////////////////////////////////
  if(_measure_cv){
    double E = _measurement(_index_tenergy); 
    _measurement(_index_cv) = E;
    _measurement(_index_cv + 1) = E * E;
  }
  // SUSCEPTIBILITY ////////////////////////////////////////////////////////////
  if(_measure_chi){
    double S=0;
    for(int i=0; i<_npart; i++){
      S += _particle(i).getspin();
    }
    _measurement(_index_chi) = S * S;
  }
...
void System :: averages(int blk){
  ...
  double sum_cv=0, sum_cv2=0;  //////added on ex6
  double sum_chi=0, sum_chi2=0;  //////added on ex6
  ...
  if (_measure_cv){
    ...
    double cv = pow(_beta,2)*(_average(_index_cv+1)-pow(_average(_index_cv),2))*_npart;
    sum_cv = pow(_beta,2)*(_global_av(_index_cv+1)/blk-pow(_global_av(_index_cv)/blk,2))*_npart;
    sum_cv2 = sum_cv*sum_cv;
    coutf << setw(12) << blk
          << setw(12) << cv
          << setw(12) << sum_cv
          << setw(12) << this->error(sum_cv, sum_cv2, blk) << endl;
    coutf.close();
  }
  // SUSCEPTIBILITY ////////////////////////////////////////////////////////////
  if (_measure_chi){
    coutf.open("../OUTPUT/susceptibility.dat",ios::app);
    double chi  = _beta*_average(_index_chi)/_npart;
    sum_chi = _beta*_global_av(_index_chi)/(blk*_npart);
    sum_chi2 = sum_chi*sum_chi;
    coutf << setw(12) << blk
          << setw(12) << chi
          << setw(12) << sum_chi
          << setw(12) << this->error(sum_chi, sum_chi2, blk) << endl;
    coutf.close();
  }
  // ACCEPTANCE ////////////////////////////////////////////////////////////////
  double fraction;
