...
void System :: initialize_velocities(){
  ...
  if(_restart){
    ...
    */
    for(int i=0; i<_npart; i+=6){ //assegno secondo una delta lungo una dir sola
      vx(i)   =   v_star;
      vy(i+1) =   v_star;
      vz(i+2) =   v_star;
      vx(i+3) = - v_star;
  ...
  if (input.is_open()){
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){
        ...
        input>>_n_bins_v;
        _nprop += _n_bins_v;
        _bin_size_v = 4.0*sqrt(_temp)/(double)_n_bins_v; // EXERCISE 4
        _measure_pofv = true;
...
void System :: measure(){ // Measure properties
  ...
  // POFV = probability of velocty ///////////////////////////////////////////// EXERCISE 4
  if(_measure_pofv){
